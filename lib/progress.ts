...
export function buildProgressSummary(user: ProgressSnapshot) {
  ...
  const dailyProgressMinutes = lastPracticeDateKey === todayKey ? user.dailyProgressMinutes : 0;
  const dailyXpToday = lastPracticeDateKey === todayKey ? user.dailyXpToday || 0 : 0;
  const dailyScenariosToday = lastPracticeDateKey === todayKey ? user.dailyScenariosToday || 0 : 0;
  const dailyGoalMinutes = Math.max(1, user.dailyGoalMinutes || 1);
  ...
  return {
    ...
    totalTimeHours: user.totalTimeHours,
    practiceRoadmap: {
      totalSessions,
      setsPerSession,
      totalSets,
...
export function applyPracticeCompletion(user: ProgressSnapshot, input: PracticeCompletionInput) {
  ...
  const nextDailyProgressMinutes = previousDateKey === currentDateKey ? user.dailyProgressMinutes + sessionMinutes : sessionMinutes;
  const previousDailyXp = previousDateKey === currentDateKey ? user.dailyXpToday || 0 : 0;
  const previousDailyScenarios = previousDateKey === currentDateKey ? user.dailyScenariosToday || 0 : 0;
  ...
  return {
    xp: user.xp + (isNewSetCompletion ? xpEarned : 0),
    streak: previousDateKey === currentDateKey ? user.streak : continuingStreak ? user.streak + 1 : 1,
    ...
    lessonsCompleted: user.lessonsCompleted + (isNewSetCompletion ? 1 : 0),
    totalTimeHours: Number((user.totalTimeHours + sessionMinutes / 60).toFixed(2)),
