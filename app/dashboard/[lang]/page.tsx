...
export default function Dashboard() {
  ...
  const selectedLanguage = userData?.targetLanguage || (session as { targetLanguage?: string } | null)?.targetLanguage || langIdStr;
  ...
  if (status === 'unauthenticated') {
    return (
      <div className={styles.pageWrapper} style={{display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh'}}>
        ...
        <button onClick={() => signIn('google')} style={{background: '#097C87', color: 'white', padding: '1rem 2rem', borderRadius: '50px', border: 'none', cursor: 'pointer', fontWeight: 'bold'}} >
          Connect with Google
        </button>
      </div>
    );
  ...
  const name = userData?.name || session?.user?.name || 'Learner';
  const avatarUrl = userData?.image || session?.user?.image || null;
      ...
      <div className={styles.dashboardContainer}>
        ...
        <div className={styles.mainGrid}>
          ...
          <div className={`${styles.whiteCard} ${styles.greetingCard}`} style={{ gridColumn: '1 / span 2', gridRow: '1' }}>
            ...
            <div className={styles.smallImageUpload} title="Profile Picture">
              {avatarUrl ? (
                 <img src={avatarUrl} className={styles.smallAvatar} alt="profile" referrerPolicy="no-referrer" />
              ) : (
                 <div className={styles.smallAvatarPlaceholder}>📷 Photo</div>
              )}
            </div>
          </div>
          ...
          <div className={styles.actionBtnGrid} style={{ gridColumn: '4', gridRow: '1 / span 2' }}>
            ...
            <button className={styles.primaryActionBtn} onClick={() => router.push(`/learn/${selectedLanguage}/practice`)}>
