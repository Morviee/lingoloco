...
export default function ChallengesPage() {
  ...
  const fetchSquads = async () => {
    try {
      ...
      const response = await fetch('/api/compete/challenges', { cache: 'no-store', signal: controller.signal });
  ...
  const handleJoinSquad = async (squadId: number) => {
    ...
    try {
      ...
      const response = await fetch('/api/compete/challenges', {
        method: 'POST',
      ...
      setBannerMessage('Joined locally. MongoDB is unavailable right now, so progress will not sync to the cloud until Atlas is reachable.');
